...
description: >
  ...
  "提取整网站的内容", 帮我读一下这篇文章", "抓取这个网页", "提取正文", or "read this page for me".
---
...
```bash
python3 <SKILL_DIR>/scripts/fetch.py "<url>" [max_chars] [--stealth] [--output-dir <dir>] [--output-name <name>]
```
...
- **`--output-dir`:** Save fetched result to a file in your specified directory (auto-creates dir)
...
python3 <SKILL_DIR>/scripts/fetch.py "https://example.com/article" --output-dir "~/Downloads/articles"

# Save with custom file name
python3 <SKILL_DIR>/scripts/fetch.py "https://example.com/article" --output-dir "./output" --output-name "my-note.md"

# Save JSON output to a specific directory
python3 <SKILL_DIR>/scripts/fetch.py "https://example.com" --json --output-dir "./output"
```

## Output Path Rule

- If user specifies an output directory, always pass `--output-dir <dir>`.
- If user also provides a target filename, also pass `--output-name <name>`.
- If user specifies `--output-name` only, save to current directory (`.`).
- If `--output-name` is not provided, the file name must use the extracted article title (not URL).
- If either `--output-dir` or `--output-name` is provided, script writes file and does not print content to stdout.
- Only print extracted content to stdout when neither output option is provided.
